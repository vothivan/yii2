...
class VerifyEmail {
	...
	public function echeck($email = ''){
		if($email != ''){
    		$this->setStreamTimeoutWait(20);
    		$this->Debug = FALSE;
    		$this->Debugoutput = 'html';
    		$this->setEmailFrom('viska@viska.is');

    		if ($this->check($email)) {
    			return ['success' => true,'email' => $email,'message' => 'Email đúng và tồn tại!'];
    		} elseif (VerifyEmail::validate($email)) {
    			return ['success' => false,'email' => $email,'message' => 'Email đúng nhưng không tồn tại!'];
    		} else {
    			return ['success' => false,'email' => $email,'message' => 'Email không đúng!'];
    		}
    	}
