app.controller('urgent', function ($scope, $uibModal) {
    $scope.customer = {
        activated: '-1',
        email: '',
        name: '',
    ...
    $scope.list = function () {
        ...
        $.ajax({
            ...
            success: function (resp) {
                ...
                if (resp.success) {
                    $scope.$apply(function () {
                        angular.forEach(resp.data.urgents, function(value, key) {
                            angular.forEach(resp.data.provinces,function (value1,key1) {
                                if(resp.data.urgents[key].provinceFromCode == resp.data.provinces[key1].provinceCode) {
                                    resp.data.customers[key].provinceFromCode = resp.data.provinces[key1].name;
                                }
                            });
                            angular.forEach(resp.data.districts,function (value1,key1) {
                                if(resp.data.urgents[key].districtFromCode == resp.data.districts[key1].districtCode) {
                                    resp.data.customers[key].districtToCode = resp.data.districts[key1].name;
                                }
                            });
                        });
